...
Q_diag = @SVector [1e-3,1e-3,1e-3, 0e-2,0e-2,0e-2,0e-2]
...
costs = map(1:N) do k
    ...
    SatDiffCost(model, Diagonal(Q_diag[iω]),
        Diagonal(@SVector fill(5e-1,3)), Diagonal(R_diag), qf, ωf)
end
obj = Objective(costs)
...
println("\nRotation type: ", Dynamics.rotation_type(model))
@btime begin
    initial_controls!($solver, $U0)
    solve!($solver)
end
...
Q_key = @SVector [1e-3,1e-3,1e-3, 1e-1,1e-1,1e-1,1e-1]
...
costs = map(1:N) do k
    if k ∈ keyframes
        ...
        SatDiffCost(model, Diagonal(Q_key[iω]),
            Diagonal(Q_key[iω .+ 3]), Diagonal(R_diag), qf, ωf)
    else
        ωf = xf[iω]
        qf = xf[iq]
        SatCost(Diagonal(Q_nom[iω]), Diagonal(R_nom), qf, 0.0, ωf)
        SatDiffCost(model, Diagonal(Q_nom[iω]),
            Diagonal(Q_nom[iω .+ 3])*0, Diagonal(R_diag), qf, ωf)
    end
end
obj = Objective(costs)
